...
qq_plot = function(pvector){
	...
	ggplot2::ggplot(df, aes(e, o)) + ggplot2::geom_point(alpha = 0.5, size = 1) + ggplot2::geom_abline(intercept = 0, slope = 1, color = '#AB3428') + ggplot2::labs(y = expression(Observed ~ ~-log[10](italic(p))), x = expression(Expected ~ ~-log[10](italic(p)))) + theme_classic() + ggplot2::annotate("text", x = 1, y = (range(o)[2]*0.9), label = paste0('lambda = ', l))
}
